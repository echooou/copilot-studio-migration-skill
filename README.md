...
| コンポーネント | 移行方法 | 注意点 |
|---|---|---|
| AI Builder プロンプト | プロンプトテキストをコピー → New の Tool または Skill にペースト | モデル選択・入出力設定は手動再構成 |
...
| コンポーネント | 理由 |
|---|---|
| **Computer Use (CUA)** | Classic（generative orchestration ON）で利用可能。New Experience での利用可否は未確認（ドキュメントに明示なし）。Tool 設定（Instructions, Machine, Credentials）はエージェント定義に埋め込まれないため、移行対象外 |
